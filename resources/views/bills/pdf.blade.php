...
<head>
    <meta charset="utf-8">
    <title>Invoice #INV-{{ str_pad($bill->id, 5, '0', STR_PAD_LEFT) }}</title>
    <style>
...
<body>
    <div class="header">
        <h1 class="title">{{ $bill->type }} INVOICE</h1>
        <p>Invoice #: <strong>INV-{{ str_pad($bill->id, 5, '0', STR_PAD_LEFT) }}</strong></p>
        <span class="status {{ $bill->status == 'paid' ? 'paid' : 'unpaid' }}"> STATUS: {{ strtoupper($bill->status) }}</span>
    </div>
    ...
    <table class="details">
        <tr>
            <td>
                ...
                <strong>{{ $bill->property->user->name }}</strong><br>
                {{ $bill->property->address }}
            </td>
            <td class="right">
                <span style="color: #64748b; font-size: 12px; font-weight: bold;">DATES</span><br>
                <strong>Billing Date:</strong> {{ $bill->billing_date }}<br>
                <strong>Due Date:</strong> <span style="color: #be123c;">{{ $bill->due_date }}</span>
            </td>
        </tr>
    </table>
    ...
    <table>
        ...
        <tfoot>
            <tr>
                ...
                <td class="right total" style="padding-top: 20px;">${{ number_format($bill->amount, 2) }}</td>
            </tr>
        </tfoot>
    </table>
