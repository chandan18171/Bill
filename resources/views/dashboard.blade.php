    ...
    <div class="py-12 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div class="bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-2xl p-6 text-white shadow-lg">
                ...
                <h3 class="text-4xl font-bold mt-2">{{ $propertiesCount }}</h3>
            </div>
            <div class="bg-gradient-to-br from-rose-500 to-red-600 rounded-2xl p-6 text-white shadow-lg">
                ...
                <h3 class="text-4xl font-bold mt-2">${{ number_format($unpaidTotal, 2) }}</h3>
            </div>
            <div class="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-2xl p-6 text-white shadow-lg">
                ...
                <h3 class="text-4xl font-bold mt-2">${{ number_format($thisMonthBills, 2) }}</h3>
            </div>
            <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl p-6 text-white shadow-lg">
                <p class="text-blue-100 text-sm font-medium uppercase tracking-wider">Billed Last Month</p>
                <h3 class="text-4xl font-bold mt-2">${{ number_format($lastMonthBills, 2) }}</h3>
                <p class="text-sm mt-1">
                    @if($thisMonthBills > $lastMonthBills) <span class="bg-rose-500 rounded px-2 py-1 text-xs">+{{ number_format((($thisMonthBills - $lastMonthBills) / ($lastMonthBills ?: 1)) * 100, 1) }}% increase</span>
                    @elseif($thisMonthBills < $lastMonthBills) <span class="bg-green-500 rounded px-2 py-1 text-xs">-{{ number_format((($lastMonthBills - $thisMonthBills) / ($lastMonthBills ?: 1)) * 100, 1) }}% decrease</span>
                    @else <span class="bg-white/20 rounded px-2 py-1 text-xs">No change</span> @endif
                </p>
            </div>
        </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            new Chart(document.getElementById('barChart').getContext('2d'), {
                ...
                data: {
                    ...
                    datasets: [
                        { label: 'Water', backgroundColor: '#3b82f6', borderRadius: 4, data: {!! json_encode($barWater) !!} },
                        { label: 'Electricity', backgroundColor: '#eab308', borderRadius: 4, data: {!! json_encode($barElec) !!} }
                    ]
                },
            ...
            new Chart(document.getElementById('lineChart').getContext('2d'), {
                type: 'line',
                data: {
                    labels: {!! json_encode($twelveMonthsLabels) !!},
                    datasets: [
                        { label: 'Water Trend', borderColor: '#3b82f6', backgroundColor: 'rgba(59, 130, 246, 0.1)', fill: true, tension: 0.4, data: {!! json_encode($waterLineData) !!} },
                        { label: 'Electricity Trend', borderColor: '#eab308', backgroundColor: 'rgba(234, 179, 8, 0.1)', fill: true, tension: 0.4, data: {!! json_encode($elecLineData) !!} }
                    ]
                },
                options: { responsive: true, maintainAspectRatio: false }
            });
